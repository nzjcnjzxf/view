(function() {
            ...
            function BgSky(config) {
                ...
                this.step = function() {
                    this.time++;
                    if(this.time % 3 == 0) { //控制背景图片height值的增加
                        this.y1++; //图片运动下一帧
                        this.y2++;
            ...
            function Loading(config) {
                ...
                this.step = function() {
                    this.time++;
                    if(this.time % 20 == 0) { //实现动画的播放速度
                        this.index++; //下一帧动画
                        if(this.index == this.sum) { //判断动画结束后，更改游戏的状态，进入第三阶段游戏阶段
                    ...
                    if(this.down) {
                        if(this.time % 15 == 0) {
                            ...
                            if(this.index >= this.sum) {
                                if(gameData.heroLife > 0) { //我方飞机生命值大于零
                                    hero = new Hero(HERO);
                                    this.down = false;
                                } else { //生命值为零
            ...
            function paintBullets() {
                for(var i = 0, length = bullets.length; i < length; i++) {
                    ...
                    if(gameData.state == gameData.RUNNING) { //游戏运行中时移动子弹
                        bullets[i].step(); //移动子弹
                    }
                }
            }
            ...
            function paintEnemiesAndCheckHit() {
                for(var i = 0; i < enemies.length; i++) { //遍历敌机
                    //
                    ...
                    enemy.paint(); //绘制飞机
                    if(gameData.state == gameData.RUNNING) { //游戏运行中时才移动飞机
                        enemy.step(); //移动飞机
                    }
                    //判断是否和我方飞机碰撞
                    if(enemy && enemy.hit(hero)) { //敌机和我方飞机相撞
                        enemy.bang();
                        hero.bang(); //飞机销毁
                    }
                    //判断子弹
                    for(var j = 0; j < bullets.length; j++) { //子弹遍历
                        var bullet = bullets[j]; //子弹
                        if(enemy.hit(bullet)) { //子弹撞机敌方飞机
                            enemy.bang(); //删除敌机
                            bullet.bang(); //删除子弹
                        }
                    }
            ...
            canvas.onclick = function() {
                if(gameData.state == gameData.START) { //在游戏开始状态下单击，进入游戏过渡阶段
