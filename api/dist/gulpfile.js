...
// 将国际化文件拷贝到目标目录
gulp.task('copy:i18n', function (done) {
    gulp.src(['src/i18n/**/*']).pipe(gulp.dest('dist/i18n')).on('end', done);
});
...
//发布
gulp.task('default', ['connect', 'copy:images', 'copy:i18n', 'fileinclude', 'md5:css', 'md5:js', 'open']);

//开发
gulp.task('dev', ['connect', 'copy:images', 'copy:i18n', 'fileinclude', 'lessmin', 'build-js', 'watch', 'open']);